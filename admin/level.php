...
<head>
  ...
  <?php include "../includes/header.php"; ?>
    <body>
        ...
        <div class="container">
            <a href="add_level.php"><h1>Rajouter un niveau</h1></a>
        </div>
            ...
            if(isset($_POST['Supprimer'])){
                if(!empty($_POST['to_delete'])){
                    foreach($_POST['to_delete'] as $selected){
                        ...
                        $sql_delete_get = "SELECT * FROM niveau WHERE `idNiveau`=" . $selected;
                        
                        $nomNiveau = "";
                        $lienCarte = "";
                        if ($result = $con->query($sql_delete_get)) {
                            if ($row = $result->fetch_assoc()){
                                $lienCarte = '<a href="' . $row["lienCarte"] . '" target="_blank">' . $row["lienCarte"] . '</a>';
                                $nomNiveau = $row["nomNiveau"];
                            }
                        }

                        $sql_delete = "DELETE FROM niveau WHERE `idNiveau`=" . $selected;
                        if (mysqli_query($con, $sql_delete)) {
                            echo "Le niveau " . $nomNiveau . " (" . $lienCarte . ") a bien été supprimé!";
                            echo "<br>";
                        } else {
                           echo "***Le niveau " . $nomNiveau . " (" . $lienCarte . ") n'a pas pu être supprimé.";
                           echo "<br>";
                        }
        ...
        ?>
            ...
            <table id="myTable">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><?php echo $row["nomNiveau"]; ?></td>
                        <td><?php echo $row["labelNiveau"]; ?></td>
                        <td><?php echo '<a href="' . $row["lienCarte"] . '" target="_blank">' . $row["lienCarte"] . '</a>'; ?></td>
                        <td>
                            <input type="checkbox" name="to_delete[]" value="<?php echo $row["idNiveau"]; ?>">
                        </td>
                    </tr>
                    <?php
                            }
        ...
        'buttons': [
            {
                ...
                action: function(e, dt, button, config) {
                    ...
                    config.title = "Niveaux";
                    config.filename = "niveaux_" + d.getDate() + "-" + (d.getMonth() + 1) + "-" + d.getFullYear();
